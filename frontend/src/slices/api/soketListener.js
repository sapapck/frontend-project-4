...
export const messagesSocketListener = async (_, { updateCachedData, cacheDataLoaded, cacheEntryRemoved }) => {
  try{
...
export const channelsSocketListener = async (_, { updateCachedData, cacheDataLoaded, cacheEntryRemoved, getState, dispatch }) => {
  try{
  await cacheDataLoaded;
  ...
  socket.on('removeChannel', (payload) => {
    ...
    dispatch(
        chatApi.util.updateQueryData('getmessages', undefined, (draftMessages) => {
          return draftMessages.filter((m) => m.channelId !== payload.id);
        })
      )
  });
